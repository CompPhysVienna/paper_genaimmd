...
def wham_position_bias(samples_cv, bias_centers, bias_potential_function, bins, beta=1.0, max_steps=100000, max_eps=1e-5, mute_warn=False):
    ...
    bins : int | float | np.ndarray | list
        ...
        must be constant.
    beta : float, optional
        The beta value to use for removing the bias potential, by default 1.0
    ...
    c = np.array([np.exp(-beta*bias_potential_function(bin_centers, bias_centers[i])) for i in range(n_simulations)])
...
def graceful_keyboard_interrupt_class(callback=None, callback_needs_self=False, message=None):
    ...
    """
    def decorator(func):
        @functools.wraps(func) # Keeps the name of the decorated method, so that it can still be pickled and looked up by name
...
def vector2D_angle(v, w):
    """Computes the angle(s) between vector(s) in 2D in radiants

    Parameters
    ----------
    v : np.ndarray | torch.Tensor
        First (set of) vector(s)
    w : np.ndarray | torch.Tensor
        Second (set of) vector(s)

    Returns
    -------
    np.ndarray | torch.Tensor | float
        The computed angle(s) between the vector(s) v and w in radiants
    """
    if isinstance(v, np.ndarray):
        b = np.sum(v*w, axis=-1)
        a = np.cross(v, w, axis=-1)
        return -np.atan2(a, b)
    else:
        b = torch.sum(v*w, axis=-1)
        c = torch.flip(w, (-1,))
        c[..., 1] *= -1
        a = torch.sum(v*c, axis=-1) # Cross product
        return -torch.atan2(a, b)
    
@jit(nopython=True)
def vector2D_angle_npy(v, w):
    """Computes the angle(s) between vector(s) in 2D in radiants only for NumPy arrays (because of Numba)
    ...
    c = w.copy()[..., ::-1]
    c[..., 1] *= -1
    a = np.sum(v*c, axis=-1) # Cross product
    return -np.atan2(a, b)
    

def vector2D_rotate(v, theta):
    """Rotates given vector(s) by angle(s) theta in 2D

    Parameters
    ----------
    v : np.ndarray | torch.Tensor
        Vector(s) to rotate
    theta : np.ndarray | torch.Tensor | float
        Angle(s) to rotate the given vector(s) by. If this is a float, the same angle will be used for all rotations
    ...
    """
    if isinstance(v, np.ndarray):
        v_rotated = np.empty_like(v)
        v_rotated[..., 0] = v[..., 0]*np.cos(theta) - v[..., 1]*np.sin(theta)
        v_rotated[..., 1] = v[..., 0]*np.sin(theta) + v[..., 1]*np.cos(theta)
    else:
        v_rotated = torch.empty_like(v)
        v_rotated[..., 0] = v[..., 0]*torch.cos(theta) - v[..., 1]*torch.sin(theta)
        v_rotated[..., 1] = v[..., 0]*torch.sin(theta) + v[..., 1]*torch.cos(theta)
    return v_rotated