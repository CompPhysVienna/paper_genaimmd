...
def generate_path_to_state_brownian(shooting_point, order_parameter, q_A, q_B, potential, timestep, diffusion_coeff, target=None, order_parameter_kwargs = {}, 
                                    beta=1.0, max_steps=10000, mute_warn=False, trajectory_out_frequency=0, return_steps_needed=False):
    ...
    max_steps : int, optional
        Number of tps steps to do at maximum, by default 10000
    ...
    trajectory_out_frequency : int, optional
        ...
        the shooting point, by default 0
    return_steps_needed : bool, optional
        If this is True, the number of steps that were performed is returned
    ...
    np.ndarray
        The trajectory with output frequency trajectory_out_frequency. This is only returned if trajectory_out_frequency > 0. It
        always includes the shooting point.

    int
        The number of steps that were performed. This is only returned if return_steps_needed is True.
    """
    if target != None:
        target = target.upper()
    ...
    if trajectory_out_frequency:
        ...
        trajectory = [shooting_point]

    position = shooting_point
    for step in range(max_steps if trajectory_out_frequency == 0 else max_steps - max_steps%trajectory_out_frequency):
        force = potential.evaluate_force_unbiased(position)
        position = brownian_propagate(position, force, timestep, diffusion_coeff, beta)

        if trajectory_out_frequency == 0 or (step+1) % trajectory_out_frequency == 0:
            ...
            if h_target or h_other:
                return_values = []
                if target != None:
                    return_values.append(h_target)
                if trajectory_out_frequency:
                    return_values.append(np.array(trajectory))
                if return_steps_needed:
                    return_values.append(step + 1)
...
def generate_path_to_state_mcmc_polymer(shooting_point, order_parameter, q_A, q_B, potential_energy_function, max_displacement, target=None, order_parameter_kwargs = {}, 
                                        beta=1.0, max_steps=10000, mute_warn=False, trajectory_out_frequency=0, return_steps_needed=False):
    """Propagates a trajectory from a given point using MCMC and observes what state is reached by the trajectory in the polymer system
    ...
    potential_energy_function : callable
        Function that takes configurations and returns their potential energy
    max_displacement : int
        Max displacement to be used in MC trial moves
    target : str, optional
        Target state, either "A" for state A or "B" for state B. If None, only trajectory will be returned, by default None
    order_parameter_kwargs : dict, optional
        Keyword arguments to pass to the order parameter, {} by default
    ...
    position = shooting_point
    current_U = potential_energy_function(position)
    for step in range(max_steps if trajectory_out_frequency == 0 else max_steps - max_steps%trajectory_out_frequency):
        new_position = position.copy()
        configs_to_reroll = np.random.random(position.shape[0]) < 1/200

        if np.sum(configs_to_reroll) > 0:
            new_position[configs_to_reroll] = Polymer.reroll_bonds(new_position[configs_to_reroll], in_place=False)

        n_configs_not_rerolled = int(np.sum(~configs_to_reroll))

        if n_configs_not_rerolled > 0:
            particles_chosen = np.random.randint(0, new_position.shape[-1]//2, n_configs_not_rerolled)
            particles_chosen = np.stack((particles_chosen*2, particles_chosen*2 + 1), axis=1).flatten()
            dr = np.random.random(n_configs_not_rerolled*2) * max_displacement
            dr *= np.random.choice([-1, 1], dr.shape)

            sub_configs = new_position[~configs_to_reroll]
            sub_configs[np.repeat(np.arange(n_configs_not_rerolled, dtype=int), 2), particles_chosen] += dr
            new_position[~configs_to_reroll] = sub_configs

            new_position[~configs_to_reroll] = Polymer.reverse(new_position[~configs_to_reroll], probability=0.1, in_place=False)
            new_position[~configs_to_reroll] = Polymer.mirror(new_position[~configs_to_reroll], probability=0.1, in_place=False)

        new_U = potential_energy_function(new_position)

        configs_to_accept = np.random.random(len(new_U)) < np.exp(-beta*(new_U - current_U))

        position[configs_to_accept] = new_position[configs_to_accept].copy()
